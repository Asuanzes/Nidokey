...
// Screen
// ──────────────────────────────────────────────────────────────────────────
export default function InmueblesIOSScreen() {
  const [current, setCurrent] = React.useState("inmuebles");
  return (
    <SafeAreaView edges={["top", "bottom"]} style={s.root}>
      {/* iOS: no backgroundColor — the native translucent bar handles it */}
      ...
      <View style={s.row}>
        ...
        <View style={s.content}>
          {/* iOS-style header: 22px BOLD title, bare + button */}
          <View style={s.header}>
            <View style={s.headerTop}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [s.iosAdd, pressed && s.iosAddPressed]}
              >
                <Plus size={22} color={T.primary} strokeWidth={2.2} />
              </Pressable>
            </View>
...
const s = StyleSheet.create({
  ...
  content: { flex: 1 },
  header: { backgroundColor: T.surface, borderBottomWidth: 1, borderColor: T.border, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 12 },
  headerTop: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" },
  // iOS large title: 22 / 700 / tight
  title: { color: T.text, fontSize: 22, fontWeight: "700", letterSpacing: -0.5, lineHeight: 26 },
  subtitle: { color: T.textMuted, fontSize: 12, marginTop: 2 },

  // iOS bare-icon + button (no fill, no border, light press feedback)
  iosAdd: { width: 36, height: 36, borderRadius: 999, alignItems: "center", justifyContent: "center", marginRight: -4 },
  iosAddPressed: { backgroundColor: T.primarySoft },
