...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (order.status !== "READY") return NextResponse.json({ error: "Estado inválido" }, { status: 409 });
  await transitionFoodOrder({
    ...
    effect: async () => {
      ...
      await sendFoodPush(id, "Pedido en camino", "El repartidor ha recogido tu pedido", userId);
    },
  });
